...
def eq_size_kmeans(points,ind,nbc,niter,random_state):
    ...
    means = KMeans(n_clusters=nbc,random_state=random_state).fit(points).cluster_centers_
    #Start iterating
    iter_ = 0
    while iter_ < niter:
...
def per_subgraph(uncon_graph,sub_cluster_num,points,random_state = 0, niter = 100):
    """Computes the same size kmeans variation for each connected sub graph of a graph.
        ...
        """
    clusters_dicts = [eq_size_kmeans(points[list(graph.nodes)],list(graph.nodes),nbc,niter,random_state = random_state) for graph,nbc in zip(nx.connected_component_subgraphs(uncon_graph),sub_cluster_num)]
