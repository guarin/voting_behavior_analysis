...
def per_subgraph(uncon_graph,sub_cluster_num,points,random_state = 0, niter = 100):
    ...
    clusters_dicts = [eq_size_kmeans(points[list(graph.nodes)],list(graph.nodes),nbc,niter,random_state = random_state) for graph,nbc in zip(subgraphs,sub_cluster_num)]
