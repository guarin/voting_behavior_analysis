...
def plot_closeness(graph,councelor_df,selected_councelor_df,title,methods):
    ...
    for ix,ax in enumerate(axes):
        closeness = np.array(list(map(lambda x :round(x[1], 2),methods(graph[ix]).items())))
        names = [write_names(x["FullName"], x['PartyAbbreviation']) for i,x in councelor_df[ix].iterrows()]
        ...
        labels = [write_names(x["FullName"], x['PartyAbbreviation']) for i,x in selected_councelor_df[ix].iterrows()]
...
def party_centrality_ranking(PartyName,ncm,pc,federal_counsilor_info):
    info = ncm.loc[ncm["PartyAbbreviation"] == PartyName]
    fc_info = federal_counsilor_info.loc[federal_counsilor_info["PartyAbbreviation"] == PartyName]
